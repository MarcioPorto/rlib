...
See https://github.com/medipixel/rl_algorithms/blob/master/algorithms/common/networks/cnn.py
"""
...
class CNNLayer:
    def __init__(self,
        ...
        """Helper class to create a CNN layer.
        """
        super(BaseCNNLayer, self).__init__()
    ...
    def forward(self, x):
        x = self.cnn(x)
        x = self.pre_activation_fn(x) if self.pre_activation_fn else x
        x = self.activation_fn(x)
        return self.post_activation_fn(x) if self.post_activation_fn else x
...
class CNN(nn.Module):
    ...
    def get_cnn_features(self, x: torch.Tensor) -> torch.Tensor:
        # If the input is 3-dimensional, unsqueeze
        if len(x.size()) == 3:
            x = x.unsqueeze(0)
        x = self.cnn(x)
        x = x.view(x.size(0), -1)
